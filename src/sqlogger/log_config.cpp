...
namespace LogConfig
{
    ...
    */
    ...
    Config Config::loadFromINI(const std::string& filename, const std::string& passKey)
    {
        ...
        if(iniData.count(LOG_INI_SECTION_TRANSPORT))
        {
            const auto& transportSection = iniData[LOG_INI_SECTION_TRANSPORT];
            if(transportSection.count(LOG_INI_KEY_TRANSPORT_TYPE))
            {
                config.transportType = TransportHelper::stringToType(transportSection.at(LOG_INI_KEY_TRANSPORT_TYPE));
            }
            if(transportSection.count(LOG_INI_KEY_TRANSPORT_HOST))
            {
                config.transportHost = transportSection.at(LOG_INI_KEY_TRANSPORT_HOST);
            }
            if(transportSection.count(LOG_INI_KEY_TRANSPORT_HOST))
            {
                if(LogHelper::isNumeric(transportSection.at(LOG_INI_KEY_TRANSPORT_PORT)))
                {
                    config.transportPort = std::stoi(transportSection.at(LOG_INI_KEY_TRANSPORT_PORT));

                }
                else
                {
                    config.transportPort = std::nullopt;
                }
            }
        }

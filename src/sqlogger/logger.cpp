 ...
 */
SQLogger::SQLogger(std::unique_ptr<IDatabase> database, const LogConfig::Config& config
...
*/
void SQLogger::log(const LogLevel level, const std::string& message)
{
 ...
 */
void SQLogger::logAdd(const LogLevel level, const std::string& message, const std::string& function, const std::string& file, int line, const std::string& threadId)
{
    if(level < config.minLogLevel.value()) return;

    std::string fileName = config.onlyFileNames.value()
                           ? FSHelper::toFilename(file)
                           : file;

#ifdef SQLG_USE_SOURCE_INFO
    if(sourceId == SOURCE_NOT_FOUND)
    {
        LOG_INTERNAL_ERROR(ERR_MSG_SOURCE_ID_NOT_INIT);
        return;
    }
#endif
 ...
 */
bool SQLogger::waitUntilEmpty(const std::chrono::milliseconds& timeout)
{
    if(config.syncMode.value())
    {
        return true;
...
*/
void SQLogger::exportTo(const std::string& filePath, const LogExport::Format& format, const LogEntryList& entryList, const std::string& delimiter, bool name)
{
    LogExport::exportTo(filePath, format, entryList, delimiter, name);
}

void SQLogger::setErrorLogPath(const std::string& errLogFile)
{
 ...
 */
void SQLogger::processTask(const LogTask& task)
{
    ...
    try
    {
        ...
        {
            success = false;
            LOG_INTERNAL_ERROR(ERR_MSG_FAILED_QUERY);
        }
    ...
    {
        success = false;
        LOG_INTERNAL_ERROR(ERR_MSG_FAILED_TASK + std::string(e.what()));
    }
 ...
 */
void SQLogger::processBatch(const std::vector<LogTask> & batch)
{
    auto startTime = std::chrono::high_resolution_clock::now();
    bool success = false;
    ...
    try
    {
        ...
        {
            success = false;
            LOG_INTERNAL_ERROR(ERR_MSG_FAILED_BATCH_QUERY);
        }
    ...
    {
        success = false;
        LOG_INTERNAL_ERROR(ERR_MSG_FAILED_BATCH_TASK + std::string(e.what()));
    }
 ...
 */
LogConfig::Config SQLogger::getConfig() const
{
 ...
 * @see setBatchSize
 */
bool SQLogger::isBatchEnabled() const
{
 ...
 * @see setBatchSize()
 */
int SQLogger::getBatchSize() const
{
 ...
 */
void SQLogger::setBatchSize(const int size)
{
 ...
 */
LogEntry SQLogger::convertTaskToEntry(const LogTask& task) const
{
    std::string fileName = config.onlyFileNames.value()
                           ? FSHelper::toFilename(task.file)
                           : task.file;
...
* @param line The line number where the log message was created.
*/
void SQLogger::logError(const std::string& errorMessage,
                       ...
                       )
{
    ...
    if(FSHelper::needLogRotation(errorLogFile))
    {
        FSHelper::rotateLog(errorLogFile);
    }

    std::ofstream errorLog(errorLogFile, std::ios::app);
    if(!errorLog.is_open())
    {
        std::cerr << ERR_MSG_FAILED_OPEN_ERR_LOG << errorLogFile << std::endl;
        return;
    }
    errorLog << getCurrentTimestamp() << " [ERROR] "
             << "\"" << errorMessage << "\""
             << " " << function
             << " " << file
             << ":" << line
 ...
 */
void SQLogger::updateSingleEntryStats(const uint64_t processTimeMs,
                                      const bool success)
{
 ...
 */
void SQLogger::updateBatchStats(const size_t batchSize,
 ...
 */
SQLogger::Stats SQLogger::getStats() const
{
    std::lock_guard<std::mutex> lock(statsMutex);
    return currentStats;
}
 ...
 * @see getStats()
 */
std::string SQLogger::getFormattedStats(const Stats stats)
{
...
* @see getStats()
*/
std::string SQLogger::getFormattedStats() const
{
    std::lock_guard<std::mutex> lock(statsMutex);
    return SQLogger::getFormattedStats(currentStats);
}
...
*/
LogEntryList SQLogger::getLogsByFilters(const std::vector<Filter> & filters,
                                        const int limit,
                                        const int offset)
{
    if(!waitUntilEmpty())
    {
        LOG_INTERNAL_ERROR(ERR_MSG_TIMEOUT_TASK_QUEUE);
    }
 ...
 */
LogEntryList SQLogger::getAllLogs(const int limit,
                                  const int offset)
{
 ...
 */
LogEntryList SQLogger::getLogsByLevel(const LogLevel& level,
                                      const int limit,
                                      const int offset)
{
    Filter filter;
 ...
 */
LogEntryList SQLogger::getLogsByTimestampRange(const std::string& startTime, const std::string& endTime,
        const int limit,
        const int offset)
{
    Filter filterStart;
 ...
 */
LogEntryList SQLogger::getLogsByFile(const std::string& file,
                                     const int limit,
                                     const int offset)
{
    Filter filter;
 ...
 */
LogEntryList SQLogger::getLogsByThreadId(const std::string& threadId,
        const int limit,
        const int offset)
{
    Filter filter;
 ...
 */
LogEntryList SQLogger::getLogsByFunction(const std::string& function,
        const int limit,
        const int offset)
{
    Filter filter;
 ...
 */
void SQLogger::setLogLevel(LogLevel minLevel)
{
 ...
 * @see setLogLevel()
 */
LogLevel SQLogger::getMinLogLevel() const
{
 ...
 * @see ThreadPool
 */
size_t SQLogger::getNumThreads() const
{
 ...
 * @see DataBaseType
 */
DataBaseType SQLogger::getDataBaseType()
{
    std::scoped_lock lock(dbMutex);
 ...
 */
bool SQLogger::isSyncMode() const
{
 ...
 */
bool SQLogger::isOnlyFileNames() const
{
 ...
 */
std::string SQLogger::getName() const
{
 ...
 */
int SQLogger::addSource(const std::string& name, const std::string& uuid)
{
    ...
    if(sourceId == SOURCE_NOT_FOUND)
    {
        LOG_INTERNAL_ERROR(ERR_MSG_FAILED_TO_ADD_SOURCE + name);
 ...
 */
std::optional<SourceInfo> SQLogger::getSourceById(const int sourceId)
{
 ...
 * @return An optional containing the source information if found, or std::nullopt otherwise.
 */
std::optional<SourceInfo> SQLogger::getSourceByUuid(const std::string& uuid)
{
 ...
 * @return An optional containing the source information if found, or std::nullopt otherwise.
 */
std::optional<SourceInfo> SQLogger::getSourceByName(const std::string& name)
{
 ...
 */
std::vector<SourceInfo> SQLogger::getAllSources()
{
...
*/
LogEntryList SQLogger::getLogsBySourceId(const int& sourceId,
        const int limit,
        const int offset)
{
    Filter filter;
...
*/
LogEntryList SQLogger::getLogsBySourceUuid(const std::string& sourceUuid,
        const int limit,
        const int offset)
{

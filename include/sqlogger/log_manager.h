...
class LogManager
{
    public:
         ...
         */
        SQLogger& createLogger(const std::string& name, const LogConfig::Config& config
         ...
         */
        SQLogger& getLogger(const std::string& name);
    ...
    private:
        ...
        std::unordered_map<std::string, std::unique_ptr<SQLogger>> loggers;
        std::mutex mutex;
};
